  ...
  return (
    <div className="card-surface rounded-2xl p-4">
      <Link href={`/squadre/${team.id}`} className="mb-4 flex items-center gap-3">
        ...
        <div>
          <h2 className="font-display text-lg font-black italic leading-tight">{team.name}</h2>
          <p className="text-xs text-outline">{roster.length} giocatori in rosa</p>
        </div>
      </Link>
      ...
      {roster.length > 0 ? (
        ...
        <p className="text-xs text-outline">Rosa non ancora inserita.</p>
      )}
    </div>
  );
}
...
export default function SquadrePage() {
  ...
  return (
    <main className="mx-auto max-w-md px-4 pb-8 pt-6">
      <h1 className="font-display mb-4 text-2xl font-black italic">Squadre</h1>
      ...
      {!loading && sortedTeams.length === 0 && (
        <p className="mt-10 text-center text-sm text-outline">Nessuna squadra ancora inserita.</p>
      )}
    </main>
  );
}
