  ...
  if (!team) {
    return (
      <main className="mx-auto max-w-md px-4 pb-8 pt-6">
        <p className="mt-10 text-center text-sm text-outline">Squadra non trovata.</p>
  ...
  return (
    <main className="mx-auto max-w-md px-4 pb-8 pt-6">
      ...
      <div className="card-surface mb-6 rounded-3xl p-6 text-center">
        ...
        <h1 className="font-display text-2xl font-black italic">{team.name}</h1>
        <p className="mt-1 text-xs text-outline">{roster.length} giocatori in rosa</p>
      </div>

      <h2 className="mb-3 text-sm font-bold uppercase tracking-wide text-on-surface-variant">Rosa</h2>
        ...
        {roster.map((p) => (
          ...
          >
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-outline">{p.role ?? "—"}</div>
            </div>
            <span className="shrink-0 text-xs font-bold text-on-surface-variant">{goalsMap.get(p.id) ?? 0} gol</span>
          </Link>
        ))}
        {roster.length === 0 && <p className="text-sm text-outline">Rosa non ancora inserita.</p>}
      </div>
    </main>
  );
}
