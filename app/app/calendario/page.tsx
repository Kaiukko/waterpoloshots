...
export default function CalendarioPage() {
  ...
  return (
    <main className="mx-auto max-w-md px-4 pb-8 pt-6">
      <h1 className="font-display mb-4 text-2xl font-black italic">Calendario</h1>
        ...
        {(["all", "Andata", "Ritorno"] as const).map((l) => (
          <button
            ...
            className={`shrink-0 rounded-full px-3.5 py-1.5 text-xs font-bold ${
              leg === l ? "bg-primary text-white" : "card-surface text-on-surface-variant"
            }`}
          >
      ...
      {!loading && filtered.length === 0 && (
        <p className="mt-10 text-center text-sm text-outline">Nessuna partita trovata con questi filtri.</p>
      )}
    </main>
  );
}
