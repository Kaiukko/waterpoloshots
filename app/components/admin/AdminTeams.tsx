  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <h2 className="font-display text-lg font-black italic">Squadre</h2>
        <button
          onClick={() => setEditing({})}
      ...
      <div className="space-y-2">
        ...
        {teams.length === 0 && <p className="text-sm text-outline">Nessuna squadra ancora.</p>}
      </div>
      ...
      {editing && (
        <Modal title={editing.id ? "Modifica Squadra" : "Nuova Squadra"} onClose={() => setEditing(null)}>
          <div className="space-y-4">
            <div>
              <label className="mb-1.5 block text-xs font-semibold text-on-surface-variant">Nome squadra</label>
              <input
                value={editing.name ?? ""}
