...
import { Player, Team, WATERPOLO_ROLES } from "@/lib/types";
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <h2 className="font-display text-lg font-bold">Giocatori</h2>
        <button
          onClick={() => setEditing({ team_id: teams[0]?.id, goals: 0 })}
          ...
          <div key={p.id} className="card-surface flex items-center gap-3 rounded-xl p-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-[#8A8A8E]">
                {teams.find((t) => t.id === p.team_id)?.name} · {p.role ?? "—"} · {p.goals} gol
              </div>
            </div>
      ...
      {editing && (
        <Modal title={editing.id ? "Modifica Giocatore" : "Nuovo Giocatore"} onClose={() => setEditing(null)}>
          <div className="space-y-4">
            ...
            />
            <div>
              <label className="mb-1.5 block text-xs font-semibold text-[#B8B8BC]">Nome</label>
              <input
                value={editing.name ?? ""}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="mb-1.5 block text-xs font-semibold text-[#B8B8BC]">Gol segnati</label>
                <input
                  type="number"
                  value={editing.goals ?? 0}
                  onChange={(e) => setEditing({ ...editing, goals: Number(e.target.value) })}
                  className="w-full rounded-lg px-3 py-2 text-sm"
                />
              </div>
            </div>
